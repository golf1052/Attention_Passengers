    ...
    for to_line in to_station_lines:
        for from_line in from_station_lines:
            if from_line['route_id'] == to_line['route_id']:
                ...
                done_transfer_chain.add_transfer(from_station, to_station, from_line['route_id'])
                        ...
                        if direction['stop'][i]['parent_station'] == transfer['station']:
                            if not transfer_chain.contains_already(transfer['station'], from_line['route_id']):
                                ...
                                if new_transfer_chain.tries > 4:
                                    # we've gone too far
                                    return None
                                new_transfer_chain.add_transfer(from_station, transfer['station'], from_line['route_id'])
...
def _retrieve_schedule_by_stop(train_id, max_trips=3):
...
class TransferChain:
    ...
    def add_transfer(self, _from, to, line):
        self.transfers.append(Transfer(_from, to, line))
...
class Transfer:
    def __init__(self, _from, to, line):
        self._from = _from
        self.to = to
        self.line = line

if __name__ == '__main__':
    get_from_to_data('wonderland', 'lechmere')